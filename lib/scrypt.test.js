...
const { encrypt, verifyPassword, encryptSync, verifyPasswordSync } = scrypt;
...
describe("scrypt", function() {
	describe("encrypt(password) => Promise(string) &nbsp; verifyPassword(password, hash) => Promise(boolean)", () => {
		...
		it("should be able to verify", async function() {
			var hash1 = await scrypt.encrypt("hello world ja");
			var verifyResult = await scrypt.verifyPassword(
				"hello world ja",
				hash1
			);
			expect(verifyResult).toBe(true);
		});
		it("should be able to verify with incorrect binding", async function() {
			var hash1 = await encrypt("hello world ja");
			var verifyResult = await verifyPassword(
				"hello world ja",
				hash1
			);
			expect(verifyResult).toBe(true);
	...
	describe("encryptSync(password) => string &nbsp; verifyPasswordSync(password, hash) => boolean", () => {
		...
		it("should be able to verify", function() {
			var hash1 = encryptSync("hello world ja");
			var verifyResult = verifyPasswordSync(
				"hello world ja",
				hash1
			);
			expect(verifyResult).toBe(true);
